package binarySearch.timeBasedKeyValueStoreLeetcode981;
...
class TimeMap {
    ...
    public String binarySearch(List<Node> list, int low, int high, int timestamp){
        String result = "";
        while(low <= high){
            int mid = low + (high - low)/2;
            Node midNode = list.get(mid);
            if(midNode.timestamp == timestamp) return midNode.value;
